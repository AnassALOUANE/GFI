...
<div id="page-heading"><h1>Ajouter Formateur</h1></div>
...
<tr>
	...
	<form:form  modelAttribute="formateur" method="post" action="${pageContext.request.contextPath}/users/f/update/${formateur.id_u}">
		...
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form">
			...
			<tr>
               <th valign="top"><form:label class="athf" path="niveau_etude"><spring:message code="label.niveau_etude"/> : </form:label></th>
                  <td>
                    	<form:select class="styledselect_form_1" path="niveau_etude">
						   <form:option value="" label="--- niveau d'etude ---"/>
						   <form:options items="${niveau_etudeList}" />
						</form:select>
						 
                   </td>
                   <td><b class="req">*</b></td>
                    <td>
					<c:set var="niveau_etudeError"><form:errors path="niveau_etude"/></c:set>
				    <c:if test="${not empty niveau_etudeError}">
				    	<div class="error-left"></div>
						<div class="error-inner"><form:errors path="niveau_etude"  /></div>
				    </c:if>
					</td>
                </tr>
		<tr>
		 
			<th valign="top"><form:label class="athf" path="experience"><spring:message code="label.experience"/> : </form:label></th>
            <td><form:input class="inp-form" path="experience" /> </td>
            <td><b class="req">*</b></td>			
			<td>
			<c:set var="experienceError"><form:errors path="experience"/></c:set>
		    <c:if test="${not empty experienceError}">
		    	<div class="error-left"></div>
				<div class="error-inner"><form:errors path="experience"  /></div>
		    </c:if>
			</td>
		</tr>
	<tr>
